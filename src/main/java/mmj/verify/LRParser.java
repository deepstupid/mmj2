...
public class LRParser implements GrammaticalParser {
    ...
    private void initialize() {
        ...
        for (final Cnst typ : grammar.getVarHypTypSet()) {
            rulesByTyp.put(typ, new ArrayList<>());
            nontermClosure.put(typ, new HashSet<>());
            initialByTyp.put(typ, new HashSet<>());
            followByTyp.put(typ, new HashSet<>());
        }
    ...
    public class ParseSet implements Collection<ParseState> {
        ...
        public Iterator<ParseState> iterator() {
            return new Iterator<>() {
                final Iterator<Cnst> iInit = initials.iterator();
                final Iterator<ParseState> iExt = extra.iterator();
                Iterator<NotationRule> iRule;
