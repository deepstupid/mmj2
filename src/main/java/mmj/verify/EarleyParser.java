    ...
    private void parse() {
        ...
        do {
            ...
            if (p > pLast || earleyCnt == 0)
                break;
            eP = ePPlus1;
            ePPlus1 = p >= pLast ? null : expr[p + 1].getCnstOrTyp();
        } while (true);
// /* --
//      dumpItemSets();
// */
...
//  }
    ...
    private void reInitArrays(final int retry) throws VerifyException {
        ...
        hardFailureMaximum = grammar.getNotationGRSet()
            .size() < GrammarConstants.EARLEY_PARSE_MIN_ITEMSET_MAXIMUM ? 2
            * GrammarConstants.EARLEY_PARSE_MIN_ITEMSET_MAXIMUM : 2 * grammar.getNotationGRSet().size();
