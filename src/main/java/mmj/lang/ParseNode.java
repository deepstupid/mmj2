...
public class ParseNode {
        ...
        final ParseNodeHolder[] expandedReseqParam)
    {
        ...
        for (int i = 0; i < out.child.length; i++)
            out.child[i] = expandedReseqParam[i] == null ? child[i].deepClone() : child[i].deepCloneWNodeSub(matchNode[i],
    ...
    public ParseNode shallowClone() {
        ...
        System.arraycopy(child, 0, out.child, 0, child.length);
        return out;
    }
    ...
    public int checkWorkVarHasOccursIn(final WorkVarHyp searchWorkVarHyp) {
        ...
        return hasOccursIn(searchWorkVarHyp) ? LangConstants.WV_OCCURS_IN_ERROR : LangConstants.WV_OCCURS_IN_NOT_AT_ALL;
    }
    ...
    public void accumSetOfWorkVarsUsed(final List<WorkVar> workVarList) {
        if (stmt instanceof WorkVarHyp) {
            ...
            for (WorkVar workVar : workVarList)
                if (workVar == v)
                    return;
            workVarList.add(v);
        }
    ...
    public static class SubTreeIterator implements Iterator<ParseNode> {
