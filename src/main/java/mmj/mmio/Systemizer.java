...
public class Systemizer {
    ...
    private Deque<IncludeFile> fileList;
    private final List<String> filesAlreadyLoaded = new ArrayList<>();

    private SrcStmt currSrcStmt;
    ...
    private final List<String> defaultProofList = new ArrayList<>(
        Collections.singletonList(MMIOConstants.MISSING_PROOF_STEP));
    ...
    private void loadTheorem() throws MMJException {
        ...
        Theorem theorem;
        if (loadProofs) {
            theorem = currSrcStmt.proofBlockList == null ? systemLoader.addTheorem(currSrcStmt.label,
                currSrcStmt.column, currSrcStmt.typ, currSrcStmt.symList,
                currSrcStmt.proofList, messages) : systemLoader.addTheorem(currSrcStmt.label,
                currSrcStmt.column, currSrcStmt.typ, currSrcStmt.symList,
        ...
        final File filePath, final String fileNameIn)
    {
        ...
        for (String s : filesAlreadyLoaded)
            if (s.equals(absPath))
                return null;
    ...
    private class LoadLimit {
        public int loadEndpointStmtNbr;
        public String loadEndpointStmtLabel;
