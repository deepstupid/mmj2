...
public enum TrUtil {
    ...
    {
        return from == 0 ? createGenBinaryNode(genStmt, left, right) : createGenBinaryNode(genStmt, right, left);
    }
    ...
    public static VarHyp findOneVarInParseNode(final ParseNode input) {
        ...
        return res.size() != 1 ? null : res.iterator().next();
    }
        ...
        final ParseNode right)
    {
        ...
        if (!implForm) {
            assert hyps.length == 0 || !info.hasImplPrefix();
