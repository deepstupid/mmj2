...
public class ImplicationInfo extends DBInfo {
        ...
        final List<Assrt> assrtList, final TrOutput output, final boolean dbg)
    {
        ...
        for (final Entry<Stmt, Assrt> entry : implOp.entrySet())
            if (!eqInfo.isEquivalence(entry.getKey()))
                if (!addPrefixRules.containsKey(entry.getKey()))
                    output.errorMessage(
                        TrConstants.ERRMSG_MISSING_IMPL_TRIV_RULE, entry.getKey(),
                        entry.getValue());
