...
public class EquivalenceInfo extends DBInfo {
    ...
    private boolean fillDeductRules;
        ...
        final ImplicationInfo implInfo)
    {
        ...
        Map<Stmt, Assrt> eqTransMap = eqDeductTrans.computeIfAbsent(implOp, k -> new HashMap<>());
    ...
    private void filterOnlyEqRules() {
        ...
        // Debug output:
        for (final Map.Entry<Stmt, Assrt> entry : eqTransitivies.entrySet())
            output.dbgMessage(dbg, TrConstants.ERRMSG_EQUIV_RULES, entry.getKey(),
                eqCommutatives.get(entry.getKey()).getFormula(),
                entry.getValue().getFormula());

        for (final Map.Entry<Stmt, Assrt> entry : eqCommutatives.entrySet()) {
            final Stmt eq = entry.getKey();
            final Assrt assrt = entry.getValue();
        ...
        final ImplicationInfo implInfo)
    {
        ...
        Map<Stmt, Assrt> eqComMap = eqDeductCom.computeIfAbsent(implOp, k -> new HashMap<>());
