...
public class AssocTree {
    ...
    public AssocTree duplicate() {
		return subTrees != null ? new AssocTree(new AssocTree[]{subTrees[0].duplicate(),
			subTrees[1].duplicate()}) : new AssocTree();
    }
        ...
        final GeneralizedStmt assocProp, final WorksheetInfo info)
    {
        ...
        for (int i = 0; i < 2; i++) {
			...
			subTrees[i] = AssociativeInfo.isAssociativeWithProp(child, assocProp, info) ? createAssocTree(child, assocProp, info) : new AssocTree();
        }
