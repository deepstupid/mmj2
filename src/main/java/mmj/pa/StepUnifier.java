...
public class StepUnifier {
        ...
        final Assrt assrt, final ParseNode stepRoot) throws VerifyException
    {
        ...
        for (Hyp hyp : assrtHypArray)
            if (hyp instanceof VarHyp)
                ((VarHyp) hyp).paSubst = null;
        ...
        final ProofStepStmt[] derivStepHypArray)
    {
        ...
        for (currLevel = 0; currLevel < assrtLogHypArray.length; currLevel++) {
            ...
            sourceRoot = derivStepHypArray[currLevel] == null ? null : derivStepHypArray[currLevel].formulaParseTree
                .getRoot();
        ...
        int levelIndex)
    {
        if (appliedCnt >= applied.length) {
            ...
            System.arraycopy(applied, 0, x, 0, appliedCnt);
            applied = x;
        }
