...
public class ProofUnifier {
    ...
    private List<Assrt> unifySearchList;
    ...
    private static final UnifyResult okUnification = new UnifyResult() {
    ...
    private static final UnifyResult badUnification = new UnifyResult() {
        public boolean proper() {
            return false;
        }
        public boolean possible() {
    ...
    private UnifyResult unifyStepWithoutWorkVars() throws VerifyException {
        ...
        return unifyHypsWithoutWorkVarsGeneralCase(assrtLogHypSubstArray) ? okUnification : badUnification;
    }
    ...
    private boolean checkAssrtLevelMatch() {
        ...
        if (!derivStep.hasDeriveStepFormula()) {
            ...
            if (!assrtLevelOneTwo.isEmpty()) {
                return assrtLevelOneTwo
                    .equals(derivStep.formulaParseTree.getLevelOneTwo()); // unification impossible!
            }
            else { // this checks LevelOne
                final Stmt assrtParseRootStmt = assrtParseTree.getRoot().stmt;

                return assrtParseRootStmt == derivStep.formulaParseTree
                    .getRoot().stmt || assrtParseRootStmt instanceof VarHyp; // unification impossible!
            }
        }
        return true;
    }
    ...
    private boolean checkHypLevelMatch() {
        ...
        if (!derivStep.hasDeriveStepHyps()) {
            ...
            // "Derive" formula.)
            return assrtLogHypsL1HiLoKey.isEmpty()
                || derivStep.getLogHypsL1HiLoKey().isEmpty() // see note
        ...
        final UnifyResult r2)
    {
        ...
        assert d2 != null;
        return d1.softDjVarsErrorList.size() <= d2.softDjVarsErrorList.size() ? r1 : r2;
    }
        ...
        final ProofStepStmt[] hypSortDerivArray, final int i)
    {
        ...
        return best == null ? badUnification : best;
    }
...
//          }
    ...
    private void saveOtherDjVarsEditResults(final DerivationStep d) {
        ...
        d.softDjVarsErrorList = holdSoftDjVarsErrorList == null
            || holdSoftDjVarsErrorList.isEmpty() ? null : new ArrayList<>(holdSoftDjVarsErrorList);
    }
        ...
        xrefLoop: for (int i = 0; i < rearrangeDerivAssrtXRef.length; i++) {
            ...
            for (int j = 0; j < origLogHypArray.length; j++)
                if (holdLogHyp == origLogHypArray[j]) {
                    ...
                    // deriveStepHyps :)
                    newHypStep[j] = newDerivStepHypArray[j] == null
                        && derivStep.hasDeriveStepHyps() ? PaConstants.DEFAULT_STMT_LABEL : newDerivStepHypArray[j].getStep();
                    continue xrefLoop;
                }
            throw new IllegalArgumentException(
    ...
    private void doUpdateDerivationStepWorkVars(final DerivationStep d) {
        ...
        // and &w3)
        ParseNode substNode;
        for (WorkVar var : updatedWorkVarList) {
            workVar = var;
        ...
        final List<List<Var>> inVarListOfLists)
    {
        ...
        loopI: for (int i = 0; i < iMax; i++) {
            ...
            final List<VarHyp> workVarHypList = new ArrayList<>(
                candidateI.size());
            for (Var var : candidateI) workVarHypList.add(var.getActiveVarHyp());
