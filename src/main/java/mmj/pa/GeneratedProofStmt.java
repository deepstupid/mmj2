...
public class GeneratedProofStmt extends ProofWorkStmt {
        ...
        final RPNStep[] rpnProof)
    {
        ...
        indentLeft.append(" ".repeat(Math.max(0, left - 1)));
        ...
        while (true) {
            if (ps < rpnProof.length) {
                if (rpnProof[ps].backRef == 0)
                    ...
                    x = String.valueOf(rpnProof[ps].backRef);
            }
        ...
        final List<Stmt> parenList, final String letters)
    {
        ...
        for (int i = 0; i <= parenList.size(); i++) {
            String label;
            label = i == parenList.size() ? ")" : parenList.get(i).getLabel();
        ...
        while (true) {
            ...
            stmtText.append(letters, lIndex, lIndex += avail);
            stmtText.append("\n");
            lineCnt++;
            stmtText.append(indentLeft);
            col = left;
