...
public interface Serializer<T> {
    ...
    static <T> Serializer<T> of(final Function<Object, T> deser,
    ...
    default Serializer<T[]> array(final IntFunction<T[]> generator) {
        return Serializer.of(
            o -> ((org.json.JSONArray)o).stream().map(this::deserialize)
                .toArray(generator),
            v -> Arrays.stream(v).map(this::serialize)
                .collect(Collectors.toCollection(org.json.JSONArray::new)));
    }
    ...
    default Serializer<Map<String, T>> map() {
        return Serializer.of(
            o -> ((org.json.JSONObject)o).entrySet().stream()
    ...
    default Serializer<Optional<T>> opt() {
        return Serializer.of(
            o -> (o == org.json.JSONObject.NULL ? Optional.empty()
                : Optional.of(deserialize(o))),
            v -> v.map(this::serialize).orElse(org.json.JSONObject.NULL));
    }

    static <T extends Enum<T>> Serializer<T> getEnumSerializer(
        final Class<T> clazz)
    {
    ...
    static Serializer<File> getFileSerializer(
    ...
    Serializer<Color> COLOR_SERIALIZER = Serializer
        .of(o -> {
            final org.json.JSONArray a = (org.json.JSONArray)o;
            try {
            ...
            final org.json.JSONArray a = new org.json.JSONArray(value.getRed(), value.getGreen(),
                value.getBlue());
            return value.getAlpha() == 255 ? a : a.put(value.getAlpha());
        });

    Serializer<Rectangle> RECT_SERIALIZER = Serializer
        .of(o -> {
            final org.json.JSONArray a = (org.json.JSONArray)o;
            try {
        ...
        }, r -> new org.json.JSONArray(r.x, r.y, r.width, r.height));

    @SuppressWarnings("unchecked")
    static <T> Serializer<T[]> getArraySerializer(final Class<T> clazz) {
        return getSerializer(clazz)
            .array(n -> (T[])Array.newInstance(clazz, n));
    }

    @SuppressWarnings("unchecked")
    static <T extends Stmt> Serializer<T> getStmtSerializer(
    ...
    @SuppressWarnings("unchecked")
    static <T extends Sym> Serializer<T> getSymSerializer(
        final LogicalSystem logicalSystem)
    {
    ...
    static <T> Serializer<T> getSerializer(final Class<T> clazz) {
        Serializer s;
