...
//            Assrt.NBR_LOG_HYP_SEQ);
        ...
        assrtAList.sort(Assrt.NBR_LOG_HYP_SEQ);
    }
        ...
        final List<Theorem> listOfAssrtAddsSortedBySeq)
    {
        ...
        addList.sort(Assrt.NBR_LOG_HYP_SEQ);
        ...
        final DerivationStep derivStep) throws VerifyException
    {
         ...
         */
        int maxHyps;
        maxHyps = nbrDerivStepHyps == derivStep.getHypNumber() ? nbrDerivStepHyps : Integer.MAX_VALUE;
        ...
        hypLoop: while (hypIndex <= maxHyps) {
            ...
            System.arraycopy(derivStepSortedHypArray, 0, derivStepHypArray, 0, nbrDerivStepHyps);
...
//          if (assrtArray[mid].getLogHypArrayLength() < nbrHyps) {
    ...
    private boolean isAssrtUnifiable() throws VerifyException {
        ...
        if (unifyStepFormulaWithWorkVars()) {
            ...
            return (assrtSubst = stepUnifier.unifyAndMergeHypsSorted(
    ...
    private boolean addAssrtToStore(final StepSelectorStore store) {
        ...
        conclusionFormula = proofAsstPreferences.stepSelectorShowSubstitutions.get() ? buildStepSelectionSubstFormula(
            assrt.getExprParseTree()) : assrt.getFormula();

        final Formula[] logHypFormula = new Formula[assrtNbrLogHyps];
        for (int i = 0; i < assrtNbrLogHyps; i++)
            logHypFormula[i] = proofAsstPreferences.stepSelectorShowSubstitutions.get() ? buildStepSelectionSubstFormula(
                assrtLogHypArray[i].getExprParseTree()) : assrtLogHypArray[i].getFormula();
