...
public class ProofWorksheet {
    ...
    public void loadComboFrameAndVarMap() {
        comboFrame = isNewTheorem() ? new ScopeFrame(logicalSystem.getScopeDefList().get(0),
            getMaxSeq()) : new ScopeFrame(theorem.getMandFrame(),
            theorem.getOptFrame());
    ...
    private void updateComboFrame() {
        if (isNewTheorem()) {
            ...
            if (hyps != null)
                comboFrame.hypArray = hyps.toArray(new Hyp[0]);
        }
        if (dvStmtCnt > 0) {
...
//      derivStepCnt              = 0;
        ...
        for (ProofDerivationStepEntry proofDerivationStepEntry : proofDerivationStepList) {
            e = proofDerivationStepEntry;
