...
public class CompiledSearchArgs {
    ...
    private void relationalCompiles() {
        ...
        if (searchThruChap != null) {
            int i = searchMaxSeq;
            i = searchThruSec != null ? bookManager.getSectionMaxMObjSeq(searchThruSec) : bookManager.getChapterMaxMObjSeq(searchThruChap);
            if (++i < searchMaxSeq)
                searchMaxSeq = i;
        }
        if (searchFromChap != null) {
            int j = searchMinSeq;
            j = searchFromSec != null ? bookManager.getSectionMinMObjSeq(searchFromSec) : bookManager.getChapterMinMObjSeq(searchFromChap);
            if (--j > searchMinSeq)
                searchMinSeq = j;
        }
    ...
    private void compileStepSearchArgs() {
        ...
        derivStepHypWildcards = nbrDerivStepHyps == derivationStep.getHypNumber() ? false : true;
