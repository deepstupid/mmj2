...
public abstract class SearchDataLine {
    ...
    {
        if (csa.searchForWhat[i].isEmpty())
            return null;
    ...
    protected boolean evaluateSearchTerms(final CompiledSearchArgs csa) {
        ...
        do {
            ...
            if (flag == quotedSearchTerm.orIsSet)
                break;
            i++;
        } while (true);
        return flag;
    ...
    protected void convertSearchTermTextToLowerCase() {
        for (QuotedSearchTerm quotedSearchTerm : quotedSearchTermList)
            quotedSearchTerm.convertSearchTermTextToLowerCase();

    }

    protected void compileSearchTermTextToRegex(final CompiledSearchArgs csa) {
        for (final QuotedSearchTerm quotedSearchTerm : quotedSearchTermList) {
            try {
