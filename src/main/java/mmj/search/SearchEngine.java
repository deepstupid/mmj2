...
public class SearchEngine {
    ...
    private void loadSearchOutput() throws InterruptedException {
        ...
        hypLoop: for (int hyp = minHyps; hyp <= maxHyps; hyp++) {
            if (stepSearchMode) {
                ...
                if (nbrDerivStepHyps >= 0) System.arraycopy(aproofStepStmt, 0, derivStepHypArray, 0, nbrDerivStepHyps);
    ...
    private boolean evaluateOtherExclusionCriteria() {
        ...
        if (bitset != null)
            if (compiledSearchArgs.searchUseChapHierarchy) {
                return bitset.get(assrt.getChapterNbr());
            }
            else return !compiledSearchArgs.searchUseSecHierarchy || bitset
                .get(BookManager.getOrigSectionNbr(assrt.getSectionNbr()));
        return true;
    }

    private boolean evaluateSearchDataLines() {
        return compiledSearchArgs.searchDataLines != null ? compiledSearchArgs.searchDataLines.evaluate(assrt,
            compiledSearchArgs) : true;
    }
    ...
    private boolean addAssrtToStore(final int score) {
        String s;
        s = score == 100 ? SearchConstants.COMPLETED_ITEM_OUTPUT_LITERAL : "";
        String s1 = "";
        int j;
